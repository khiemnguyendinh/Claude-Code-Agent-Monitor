...
import { KadButton, KadCard, KadFieldLabel, KadInput, KadTextarea } from "../../components/primitives";
...
type WizardData = OrgContextData & {
  ...
  _templates: { name: string; file_name?: string; template_type: string; purpose: string; content: string }[];
};
...
export function OrgContextWizard({ onCompleted }: { onCompleted: () => void }) {
  ...
  const saveDraft = async (nextStep = step) => {
    ...
    setError(null);
    try {
      await kadApi.orgContext.saveWizardDraft({ step: nextStep + 1, draft });
    } catch (e) {
  ...
  const complete = async () => {
    ...
    setError(null);
    try {
      await kadApi.orgContext.saveWizardDraft({ step: STEPS.length, draft });
      await kadApi.orgContext.completeWizard({ draft });
  ...
  return (
    <div className="max-w-[980px] space-y-5">
      ...
      <div className="flex items-center justify-between">
        <KadButton variant="ghost" disabled={step === 0 || saving} onClick={() => setStep((s) => Math.max(0, s - 1))}>
          Quay lại
        </KadButton>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <Field label="Tên tổ chức">
        <KadInput value={draft._profile.name} onChange={(e) => patchDraft({ _profile: { ...draft._profile, name: e.target.value } })} />
      </Field>
      <Field label="Lĩnh vực">
        <KadInput value={draft._profile.industry} onChange={(e) => patchDraft({ _profile: { ...draft._profile, industry: e.target.value } })} />
      </Field>
      <Field label="Quy mô">
        <select
          value={draft._profile.size}
          onChange={(e) => patchDraft({ _profile: { ...draft._profile, size: e.target.value as WizardData["_profile"]["size"] } })}
  ...
  return (
    <div className="space-y-4">
      <Field label="Tầm nhìn">
        <KadTextarea value={draft.vision} onChange={(e) => patchDraft({ vision: e.target.value })} />
      </Field>
      <Field label="Sứ mệnh">
        <KadTextarea value={draft.mission} onChange={(e) => patchDraft({ mission: e.target.value })} />
      </Field>
      <Field label="Giá trị cốt lõi">
        <KadTextarea value={joinLines(draft.core_values)} onChange={(e) => patchDraft({ core_values: splitLines(e.target.value) })} />
      </Field>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <Field label="Brand voice">
        <KadTextarea value={draft.brand.voice} onChange={(e) => patchDraft({ brand: { ...draft.brand, voice: e.target.value } })} className="min-h-[120px]" />
      </Field>
      <Field label="Guideline">
        <KadTextarea value={draft.brand.guideline} onChange={(e) => patchDraft({ brand: { ...draft.brand, guideline: e.target.value } })} className="min-h-[120px]" />
      </Field>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Field label="Màu chính">
          <KadInput value={draft.brand.primary_color ?? ""} onChange={(e) => patchDraft({ brand: { ...draft.brand, primary_color: e.target.value } })} />
        </Field>
        <Field label="Font">
          <KadInput value={draft.brand.font ?? ""} onChange={(e) => patchDraft({ brand: { ...draft.brand, font: e.target.value } })} />
        </Field>
        <Field label="Slogan">
          <KadInput value={draft.brand.slogan ?? ""} onChange={(e) => patchDraft({ brand: { ...draft.brand, slogan: e.target.value } })} />
        </Field>
      </div>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      <Field label="Tên sản phẩm">
        <KadInput value={p.name} onChange={(e) => patchDraft({ products: [{ ...p, name: e.target.value }] })} />
      </Field>
      <Field label="Mô tả">
        <KadTextarea value={p.description} onChange={(e) => patchDraft({ products: [{ ...p, description: e.target.value }] })} />
      </Field>
      <Field label="Đối tượng">
        <KadInput value={p.target_audience} onChange={(e) => patchDraft({ products: [{ ...p, target_audience: e.target.value }] })} />
      </Field>
    </div>
  );
}
...
function StepPersona({ draft, patchDraft }: StepProps) {
  const p = draft.personas[0] ?? { name: "", demographics: "", needs: "", pain_points: "", channels: "" };
  const update = (patch: Partial<(typeof draft.personas)[number]>) => patchDraft({ personas: [{ ...p, ...patch }] });
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <Field label="Persona">
      ...
      <Field label="Demographics">
        <KadInput value={p.demographics} onChange={(e) => update({ demographics: e.target.value })} />
      </Field>
      <Field label="Nhu cầu">
        <KadTextarea value={p.needs} onChange={(e) => update({ needs: e.target.value })} />
      </Field>
      <Field label="Pain points">
        <KadTextarea value={p.pain_points} onChange={(e) => update({ pain_points: e.target.value })} />
      </Field>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {(["goals", "priorities", "constraints", "roadmap"] as const).map((key) => (
        <Field key={key} label={key}>
          <KadTextarea value={draft.strategy[key]} onChange={(e) => patchDraft({ strategy: { ...draft.strategy, [key]: e.target.value } })} />
        </Field>
      ))}
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {(["strengths", "weaknesses", "opportunities", "threats"] as const).map((key) => (
        <Field key={key} label={key}>
          <KadTextarea value={joinLines(draft.swot[key])} onChange={(e) => patchDraft({ swot: { ...draft.swot, [key]: splitLines(e.target.value) } })} />
        </Field>
      ))}
    </div>
  );
...
function StepCompetitors({ draft, patchDraft }: StepProps) {
  const text = draft.competitors.map((c) => [c.name, c.strengths, c.weaknesses, c.differentiator].join(" | ")).join("\n");
  return (
    <Field label="Đối thủ">
      <KadTextarea
        value={text}
        ...
        onChange={(e) =>
          patchDraft({
            competitors: splitLines(e.target.value).map((line) => {
              const [name = "", strengths = "", weaknesses = "", differentiator = ""] = line.split("|").map((s) => s.trim());
  ...
  return (
    <div className="space-y-2">
      {draft._org_chart_nodes.map((node) => (
        <div key={node.id} className="grid grid-cols-1 sm:grid-cols-[1fr_160px_1fr] gap-2 border border-kad-border rounded-lg p-3">
          <KadInput value={node.name} onChange={(e) => updateNode(node.id, { name: e.target.value })} />
          <select
            value={node.node_type}
            onChange={(e) => updateNode(node.id, { node_type: e.target.value as WizardData["_org_chart_nodes"][number]["node_type"] })}
          ...
          <KadInput value={node.parent_id ?? ""} placeholder="parent id" onChange={(e) => updateNode(node.id, { parent_id: e.target.value || null })} />
        </div>
      ))}
      <KadButton
        variant="ghost"
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <Field label="Tên phòng ban">
        <KadInput value={draft._department.name} onChange={(e) => patchDraft({ _department: { ...draft._department, name: e.target.value } })} />
      </Field>
      <Field label="Template">
        <select
          value={draft._department.template_type}
          onChange={(e) => patchDraft({ _department: { ...draft._department, template_type: e.target.value as WizardData["_department"]["template_type"] } })}
          className="kad-body h-9 w-full rounded-lg bg-kad-surface-2 px-3 text-kad-text"
        >
      ...
      <div className="sm:col-span-2">
        <Field label="Mission">
          <KadTextarea value={draft._department.mission} onChange={(e) => patchDraft({ _department: { ...draft._department, mission: e.target.value } })} />
        </Field>
      </div>
    </div>
  );
...
function StepTemplates({ draft, patchDraft }: StepProps) {
  ...
  return (
    <div className="space-y-3">
      <p className="kad-body text-kad-text-muted">Seed mặc định gồm 6 template R&D. File upload sẽ thêm vào thư viện.</p>
      ...
      {draft._templates.map((tpl) => (
        <div key={`${tpl.file_name}-${tpl.content.length}`} className="border border-kad-border rounded-lg px-3 py-2">
          <p className="kad-body text-kad-text">{tpl.name}</p>
          <p className="kad-caption text-kad-text-faint">{tpl.content.length.toLocaleString("vi-VN")} ký tự</p>
        </div>
      ))}
    </div>
  );
