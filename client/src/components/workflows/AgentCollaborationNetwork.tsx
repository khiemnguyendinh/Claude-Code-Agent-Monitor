...
// 10-slot categorical palette for agent-type nodes (colorIndex cycles through
// these). First 4 reuse kad tokens (accent/primary/info/text-muted); remaining
// 6 are new muted/desaturated hex chosen to stay distinct from each other and
// from the kad tokens already used.
const PALETTE = [
  "var(--kad-accent)",
  "var(--kad-primary)",
  "var(--kad-info)",
  "var(--kad-text-muted)",
  "#a35a2f", // muted terracotta/brown
  "#6d3fae", // muted purple
  "#b0518a", // muted magenta/rose
  "#3f7a5c", // muted teal-green
  "#8a7a1f", // muted olive/gold
  "#4a6fa5", // muted steel blue
];

// Stroke ring for each node uses the same family color, at full opacity
// (fill-opacity on the circle already provides the lighter wash).
const STROKE_PALETTE = PALETTE;
...
function appendTooltipRow(parent: HTMLElement, label: string, value: string) {
  ...
  val.style.cssText = "color:var(--kad-text);font-weight:500";
...
function appendTooltipDescription(parent: HTMLElement, text: string) {
  ...
  p.style.cssText =
    "font-size:11px;color:var(--kad-text-muted);line-height:1.45;margin:8px 0 0;padding-top:8px;border-top:1px solid var(--kad-border)";
  p.textContent = text;
...
function showTooltip(
  ...
  title.style.cssText = "font-size:12px;font-weight:600;color:var(--kad-text-strong);margin:0 0 2px";
...
export function AgentCollaborationNetwork({
  ...
  useEffect(() => {
    ...
    const linkEls = linkGroup
      ...
      .join("path")
      .attr("fill", "none")
      .attr("stroke", "var(--kad-text-muted)")
    ...
    nodeEls
      ...
      .attr("fill", (d) => PALETTE[d.colorIndex] ?? "var(--kad-accent)")
      .attr("fill-opacity", 0.8)
      .attr("stroke", (d) => STROKE_PALETTE[d.colorIndex] ?? "var(--kad-accent)")
      .attr("stroke-width", 2);
    ...
    linkHits
      .on("mouseenter", (event: MouseEvent, d: PipelineLink) => {
        ...
        if (tipEl) {
          ...
          const title = document.createElement("p");
          title.style.cssText =
            "font-size:12px;font-weight:600;color:var(--kad-text-strong);margin:0 0 2px";
  ...
  if (isEmpty) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <p className="text-sm font-medium text-[var(--kad-text-muted)]">{t("pipeline.noData")}</p>
        <p className="text-xs text-[var(--kad-text-faint)] mt-1">{t("pipeline.noDataDesc")}</p>
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="w-full relative" onMouseLeave={handleContainerLeave}>
      ...
      <div className="flex flex-wrap items-center gap-3 mt-3 px-1">
        <span className="text-[10px] text-[var(--kad-text-muted)] uppercase tracking-widest font-medium">
          {t("pipeline.legend")}
        </span>
        {nodes.map((n) => (
          <div key={n.id} className="flex items-center gap-1.5">
            ...
            />
            <span className="text-[11px] text-[var(--kad-text-muted)]">{n.id}</span>
          </div>
        ))}
        <div className="flex items-center gap-1.5 ml-2">
          <svg width="20" height="8" className="flex-shrink-0">
            <line x1="0" y1="4" x2="14" y2="4" stroke="var(--kad-text-muted)" strokeWidth="1.5" />
            <polygon points="14,1 20,4 14,7" fill="var(--kad-text-muted)" />
          </svg>
          <span className="text-[11px] text-[var(--kad-text-muted)]">{t("pipeline.legendDesc")}</span>
        </div>
      </div>
    </div>
  );
