  ...
  return (
    <label className="flex items-center justify-between gap-3 cursor-pointer group">
      <div className="min-w-0">
        <p className="text-sm text-kad-text transition-colors">{label}</p>
        {description && <p className="text-xs text-kad-text-muted mt-0.5">{description}</p>}
      </div>
      <button
        type="button"
        role="switch"
      ...
      >
        <span
          className={`pointer-events-none inline-block h-4 w-4 rounded-full bg-white transform transition-transform duration-200 ${
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center justify-center rounded-full p-0.5 text-kad-text-muted hover:text-kad-text focus:outline-none focus:ring-1 focus:ring-accent/40"
      >
      ...
      {open && (
        <div
          ref={popoverRef}
          role="tooltip"
          className="fixed z-50 p-3 bg-kad-surface border border-kad-border-strong rounded-lg text-[11px] text-kad-text pointer-events-none"
          style={{ left: pos.left, top: pos.top, width: 320, boxShadow: "var(--kad-shadow-1)" }}
        >
          <p className="text-xs font-semibold text-kad-text-strong mb-2">
            {t("pricing.tooltip.title")}
          </p>

          <p className="font-semibold text-kad-text-muted uppercase tracking-wider text-[9px] mb-1">
            {t("pricing.tooltip.howItWorks")}
          </p>
          <p className="text-kad-text-muted leading-snug mb-2.5">
          ...
          <p className="text-kad-text-muted leading-snug">{t("pricing.tooltip.apiPricingBody")}</p>
        </div>
      )}
    </>
  );
...
// 2026-07-04: "Setting" in the unified sidebar carries a top-level tab bar
// (CC Config | Run | Settings) per Khiêm's spec — the standalone "CC Config"
// and "Run" nav items are folded in here as tabs. All pre-existing settings
// logic stays untouched in `SettingsInner`.
    ...
    <>
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={cancelEdit}
            className="p-1.5 rounded-md text-kad-text-muted hover:bg-surface-4 transition-colors"
      ...
      <td colSpan={10} className="px-4 pb-3 pt-2">
        <div className="mt-2 rounded-md border border-violet-500/20 bg-violet-500/[0.04] px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-[11px] text-kad-text-muted">{t("pricing.introRatesHint")}</span>
          </div>
  ...
  return (
    <div className="animate-fade-in space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <a
            ...
            download
            className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md border border-border text-kad-text-muted hover:text-kad-text hover:border-kad-border-strong transition-colors"
          >
      ...
      <nav className="sticky top-0 z-20 -mx-1 !mt-2 px-1 py-2 bg-surface-0/85 backdrop-blur border-b border-border/60 flex items-center gap-1.5">
        <span className="flex-shrink-0 pl-1 text-[10px] font-semibold uppercase tracking-wider text-kad-text-muted">
          {t("jumpTo", "Jump to")}
        </span>
        {tocOverflow.left && (
          <button
            ...
            className="flex-shrink-0 flex items-center justify-center w-6 h-7 rounded-md border border-border text-kad-text-muted hover:text-kad-text hover:bg-surface-3 transition-colors"
          >
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-kad-text-muted">{t("common:cost.totalEstimatedCost")}</p>
              <p className="text-2xl font-semibold text-kad-text-strong">
                <Tip
                  raw={
                    totalCost !== null
          ...
          <div className="text-right text-xs text-kad-text-muted">
      ...
      <section id="pricing" className="scroll-mt-24">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div>
            <h3 className="text-sm font-medium text-kad-text flex items-center gap-2">
              <DollarSign className="w-4 h-4 text-kad-text-muted" />
              {t("pricing.title")}
              <PricingInfoTooltip />
            </h3>
            <p className="text-xs text-kad-text-muted mt-0.5">{t("pricing.description")}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() =>
              ...
              className={`text-xs px-3 py-1.5 rounded-md transition-colors disabled:opacity-50 inline-flex items-center gap-1.5 ${
                confirmAction === "reset-pricing"
                  ...
                  : "text-kad-text-muted hover:text-kad-text hover:bg-surface-4"
              }`}
            >
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="border-b border-border text-left">
                <th className="px-4 py-3 text-[11px] font-semibold text-kad-text-muted uppercase tracking-wider">
                  {t("pricing.pattern")}
                </th>
                <th className="px-4 py-3 text-[11px] font-semibold text-kad-text-muted uppercase tracking-wider">
                  {t("common:cost.model")}
                </th>
                <th className="px-4 py-3 text-[11px] font-semibold text-kad-text-muted uppercase tracking-wider text-right">
                ...
                <th className="w-24 px-4 py-3 text-[11px] font-semibold text-kad-text-muted uppercase tracking-wider">
                  {t("common:actions")}
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {pricing.map((rule) =>
                editingPattern === rule.model_pattern ? (
                  ...
                  >
                    <td className="px-4 py-3 text-sm font-mono text-kad-text">
                      {rule.model_pattern}
                    </td>
                    <td className="px-4 py-3 text-sm text-kad-text">
                      {rule.display_name}
                      {rule.intro_until && (
                    ...
                    <td className="px-4 py-3 text-sm text-kad-text-muted text-right font-mono">
                      ${rule.input_per_mtok}
                      {rule.intro_until && (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 transition-opacity">
                        <button
                          ...
                          disabled={isEditing}
                          className="p-1.5 rounded-md text-kad-text-muted hover:text-blue-400 hover:bg-blue-500/10 transition-colors disabled:opacity-30"
                          title={t("common:edit")}
                        >
                        ...
                        <button
                          ...
                          disabled={isEditing}
                          className="p-1.5 rounded-md text-kad-text-muted hover:text-red-400 hover:bg-red-500/10 transition-colors disabled:opacity-30"
        ...
        {lastUpdated && (
          <p className="text-xs text-kad-text-muted mt-3">
      ...
      <section id="hooks" className="scroll-mt-24">
        <h3 className="text-sm font-medium text-kad-text flex items-center gap-2 mb-1">
          <Plug className="w-4 h-4 text-kad-text-muted" />
          {t("hooks.title")}
        </h3>
        <p className="text-xs text-kad-text-muted mb-4">{t("hooks.description")}</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                {Object.entries(sysInfo.hooks.hooks).map(([hook, active]) => (
                  ...
                  >
                    ...
                    <span className="text-kad-text-muted truncate">{hook}</span>
                  </div>
                ))}
              </div>
              <p className="text-[11px] text-kad-text-muted font-mono truncate">{sysInfo.hooks.path}</p>
            </>
          )}
        </div>
      </section>
      ...
      <section id="claude-home" className="scroll-mt-24">
        <h3 className="text-sm font-medium text-kad-text flex items-center gap-2 mb-1">
          <FolderOpen className="w-4 h-4 text-kad-text-muted" />
          {t("claudeHome.title")}
        </h3>
        <p className="text-xs text-kad-text-muted mb-4">{t("claudeHome.description")}</p>
        ...
        <div className="card p-5 space-y-4">
          <div className="flex items-center gap-3">
            <input
              ...
              className="flex-1 bg-surface-4 border border-surface-3 rounded-lg px-3 py-2 text-sm text-kad-text-strong font-mono focus:outline-none focus:border-violet-500/50"
          ...
          {claudeHome && (
            <p className="text-xs text-kad-text-muted">
              {t("claudeHome.current")} <code className="text-kad-text-muted">{claudeHome}</code>
            </p>
          )}
        </div>
      </section>
      ...
      <section id="tabby" className="scroll-mt-24">
        ...
        <p className="text-xs text-kad-text-muted mb-4">
      ...
      <section id="notifications" className="scroll-mt-24">
        <h3 className="text-sm font-medium text-kad-text flex items-center gap-2 mb-1">
          <Bell className="w-4 h-4 text-kad-text-muted" />
          {t("notifications.title")}
        </h3>
        <p className="text-xs text-kad-text-muted mb-4">{t("notifications.description")}</p>
        ...
        <div className="card p-5 space-y-5">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {notifPrefs.enabled ? (
                  ...
                  <BellOff className="w-5 h-5 text-kad-text-muted" />
                )}
              </div>
              <Toggle
          ...
          {notifPrefs.enabled && (
            <div className="space-y-3 pt-4 border-t border-border">
              <p className="text-xs text-kad-text-muted uppercase tracking-wider font-semibold">
              ...
              <div className="pt-3 border-t border-border">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md text-kad-text-muted hover:text-kad-text hover:bg-surface-4 border border-border transition-colors"
                >
      ...
      <section id="alerts" className="scroll-mt-24">
        <h3 className="text-sm font-medium text-kad-text flex items-center gap-2 mb-1">
          <BellRing className="w-4 h-4 text-kad-text-muted" />
          {t("alertsHub.title")}
        </h3>
        <p className="text-xs text-kad-text-muted mb-4">{t("alertsHub.description")}</p>
        <AlertsNotifications />
      </section>
      ...
      <section id="data" className="scroll-mt-24">
        <h3 className="text-sm font-medium text-kad-text flex items-center gap-2 mb-1">
          <Database className="w-4 h-4 text-kad-text-muted" />
          {t("data.title")}
        </h3>
        <p className="text-xs text-kad-text-muted mb-4">{t("data.description")}</p>

        <div className="space-y-4">
          <div className="card p-5 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-xs text-kad-text-muted uppercase tracking-wider font-semibold flex-shrink-0">
                {t("data.dbOverview")}
              </p>
              {sysInfo && (
                <div className="flex items-center gap-1.5 text-[11px] text-kad-text-muted font-mono bg-surface-2 px-2.5 py-1 rounded-md min-w-0">
            ...
            {sysInfo ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                {(() => {
                  ...
                  return Object.entries(sysInfo.db.counts).map(([table, count]) => (
                    <div
                      key={table}
                      className={`bg-surface-2 rounded-lg px-3 py-3 border-l-2 ${tableColors[table] || "border-kad-border-strong"}`}
                    >
                      <div className="flex items-center gap-2 mb-1.5">
                        {tableIcons[table] || <Database className="w-4 h-4 text-kad-text-muted" />}
                        <p className="text-[11px] text-kad-text-muted uppercase tracking-wider">
                          {tableLabels[table] || table.replace(/_/g, " ")}
                        </p>
                      </div>
                      <p className="text-xl font-semibold text-kad-text-strong">
              ...
              <p className="text-xs text-kad-text-muted">{t("data.loadingDb")}</p>
            )}
          </div>
          ...
          <div className="card p-5 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-kad-text">{t("data.sessionCleanup")}</p>
                <p className="text-xs text-kad-text-muted">{t("data.cleanupDesc")}</p>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-surface-2 rounded-lg px-4 py-3">
                <label className="text-xs text-kad-text-muted block mb-2">{t("data.abandonAfter")}</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min="1"
                  ...
                  />
                  <span className="text-xs text-kad-text-muted">{t("common:hours")}</span>
                </div>
              </div>
              <div className="bg-surface-2 rounded-lg px-4 py-3">
                <label className="text-xs text-kad-text-muted block mb-2">{t("data.purgeAfter")}</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min="1"
                    ...
                    className="input w-20 text-sm text-right font-mono"
                  />
                  <span className="text-xs text-kad-text-muted">{t("common:days")}</span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-md transition-colors disabled:opacity-50 ${
                confirmAction === "cleanup"
                  ? "bg-amber-500/20 text-amber-400 border border-amber-500/30"
                  : "text-kad-text-muted hover:text-kad-text hover:bg-surface-4 border border-border"
              }`}
            >
          ...
          <div className="card p-5 space-y-4 border-red-500/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-kad-text-muted">{t("danger.description")}</p>
              </div>
            </div>
              ...
              <div className="bg-red-500/5 border border-red-500/20 rounded-lg px-4 py-3 flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="text-xs px-3 py-1.5 rounded-md text-kad-text-muted hover:bg-surface-4 transition-colors"
                  >
                    {t("common:cancel")}
                  </button>
                </div>
      ...
      <section id="about" className="scroll-mt-24">
        <h3 className="text-sm font-medium text-kad-text flex items-center gap-2 mb-1">
          <Server className="w-4 h-4 text-kad-text-muted" />
          {t("about.title")}
        </h3>
        <p className="text-xs text-kad-text-muted mb-4">{t("about.description")}</p>
        ...
        {sysInfo ? (
          <div className="card p-5">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-surface-2 rounded-lg px-4 py-3">
                ...
                <p className="text-sm font-semibold text-kad-text-strong">
              ...
              <div className="bg-surface-2 rounded-lg px-4 py-3">
                ...
                <p className="text-sm font-semibold text-kad-text-strong font-mono">
              ...
              <div className="bg-surface-2 rounded-lg px-4 py-3">
                ...
                <p className="text-sm font-semibold text-kad-text-strong">{sysInfo.server.platform}</p>
              </div>
              <div className="bg-surface-2 rounded-lg px-4 py-3">
                <div className="flex items-center gap-2 mb-1.5">
          ...
          <p className="text-xs text-kad-text-muted">{t("about.loadingInfo")}</p>
        )}
      </section>
    </div>
  );
...
const SETTING_TABS = [
  { key: "cc-config", label: "CC Config" },
  { key: "run", label: "Run" },
  { key: "settings", label: "Settings" },
];
const SETTING_TAB_KEY = "setting-page-tab";

function loadSettingTab(): string {
  try {
    const stored = localStorage.getItem(SETTING_TAB_KEY);
    if (stored === "cc-config" || stored === "run" || stored === "settings") return stored;
  } catch {
    /* ignore */
  }
  return "settings";
...
export function Settings() {
  const [tab, setTab] = useState(loadSettingTab);

  const changeTab = (key: string) => {
    setTab(key);
    try {
      localStorage.setItem(SETTING_TAB_KEY, key);
    } catch {
      /* ignore */
    }
  };
  ...
  return (
    <div className="animate-fade-in">
      <Tabs items={SETTING_TABS} active={tab} onChange={changeTab} />
      <div className="mt-6">
        {tab === "cc-config" && <CcConfig />}
        {tab === "run" && <Run />}
