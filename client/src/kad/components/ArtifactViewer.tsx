...
export function ArtifactViewer({ artifactId }: { artifactId: string }) {
  ...
  const hasSensitivity = artifact.sensitivityFlags.metrics || artifact.sensitivityFlags.people || artifact.sensitivityFlags.brand;
    ...
    <div className="space-y-4">
      <div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {artifact.qualityScore !== null && (
            <span className="kad-caption text-kad-success font-medium">{artifact.qualityScore.toFixed(1)}/10</span>
          )}
        </div>
            ...
            {versions.map((v) => (
              <span
                ...
                className={`kad-caption px-2 py-1 rounded-md ${
                  v.id === artifact.id ? "bg-kad-surface-2 text-kad-text-strong font-medium" : "text-kad-text-muted"
                }`}
              >
                v{v.version}
              </span>
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-kad-border">
        {artifact.status === "review" && (
          <>
            <KadButton variant="primary" onClick={() => toast({ message: "Đã duyệt học liệu.", tone: "success" })}>
              Duyệt
            </KadButton>
            <KadButton variant="secondary" onClick={() => toast({ message: "Đã gửi yêu cầu sửa.", tone: "warning" })}>
              Yêu cầu sửa
            </KadButton>
          </>
        )}
        {artifact.status === "approved" && (
          <KadButton variant="secondary" onClick={() => toast({ message: "Kết nối xuất bản chưa nằm trong phạm vi mockup." })}>
            Xuất bản…
          </KadButton>
        )}
        <KadButton
