...
export function JDKyNangTab() {
  ...
  const proposeBlueprint = (summary: string) => {
    ...
    kadApi.orgContext
      ...
      .catch((e) => toast({ message: e instanceof Error ? e.message : "Không gửi được blueprint.", tone: "warning" }));
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6">
      <div className="border border-kad-border rounded-xl overflow-hidden h-fit">
        {agents.map((a) => (
          ...
          >
            <AgentAvatar agentId={a.id} size={20} displayNameOverride={a.displayName} agentNameOverride={a.name} />
        ...
        <div className="space-y-6 max-w-[720px]">
          {pending && (
            <div className="flex items-center justify-between gap-3 rounded-lg bg-[#fbf3e4] px-3 py-2">
              <p className="kad-caption text-[#99690f]">Blueprint v{pending.version} đang chờ duyệt.</p>
              <KadButton
                variant="primary"
                size="row"
                onClick={() => {
                  kadApi.orgContext
                    ...
                    .catch((e) => toast({ message: e instanceof Error ? e.message : "Không duyệt được blueprint.", tone: "warning" }));
                }}
              >
                Duyệt
              </KadButton>
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              ...
              <KadButton variant="ghost" size="row" onClick={() => proposeBlueprint(`Đề xuất sửa JD cho ${agent.name}`)}>
                Đề xuất sửa
              </KadButton>
            </div>
            <MarkdownLite content={agent.roleDescription || "(chưa có JD)"} />
            <p className="kad-caption text-kad-text-faint mt-2">
              Blueprint v{approved?.version ?? "-"} · duyệt {approved?.approved_at ? new Date(approved.approved_at).toLocaleDateString("vi-VN") : "chưa có"}
            </p>
          </section>
          ...
          <section className="pt-4 border-t border-kad-border">
            <div className="flex items-center justify-between mb-2">
              ...
              <KadButton variant="ghost" size="row" icon={Plus} onClick={() => proposeBlueprint(`Đề xuất thêm kỹ năng cho ${agent.name}`)}>
                Thêm kỹ năng
              </KadButton>
            </div>
            <div className="space-y-2">
              {visibleTemplates.length === 0 ? (
                <p className="kad-body text-kad-text-faint">Agent này chưa được cấp quyền đọc template.</p>
              ) : (
                visibleTemplates.map((tpl) => (
                  <div key={tpl.id} className="flex items-center justify-between gap-3 border border-kad-border rounded-lg px-3 py-2.5">
                    <div className="min-w-0">
                      <p className="kad-body text-kad-text truncate">{tpl.name}</p>
                      <p className="kad-caption text-kad-text-muted truncate">
                        {tpl.template_type} · v{tpl.approved_version?.version ?? "-"} · {tpl.usage_count} lượt dùng
                      </p>
                    </div>
                    <label className="inline-flex items-center gap-2 flex-shrink-0 cursor-pointer">
                      <input
                        ...
                        checked
                        readOnly
                        onClick={() => proposeBlueprint(`Đề xuất đổi quyền template ${tpl.name} cho ${agent.name}`)}
          ...
          <section className="pt-4 border-t border-kad-border">
            ...
            <KadButton variant="ghost" size="row" className="mt-3" onClick={() => proposeBlueprint(`Đề xuất sửa engine cho ${agent.name}`)}>
              Gửi duyệt thay đổi
            </KadButton>
          </section>
        </div>
