  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <div className="w-14 h-14 rounded-2xl bg-surface-4 flex items-center justify-center mb-5">
        <Icon className="w-6 h-6 text-kad-text-muted" />
      </div>
      <h3 className="text-base font-medium text-kad-text mb-2">{title}</h3>
      <p className="text-sm text-kad-text-muted max-w-md mb-6">{description}</p>
      {action}
    </div>
  );
}
